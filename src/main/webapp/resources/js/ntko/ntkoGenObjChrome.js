...
function OnComplete2(type,code,html)
{
  ...
  if (code == 0) {
    // parent.parent.layer.alert('保存成功！',{offset: '10px',});
    parent.parent.layer.msg('保存成功！',{time: 2000});
    $(window.parent.document.getElementById('LAY_app_tabsheader')).children(".layui-this").children(".layui-tab-close").trigger("click");
  } else {
    parent.parent.layer.alert('保存失败！',{offset: '10px'});
  }
...
function OnComplete(type,code,html)
{
  ...
  setFileOpen(false);//禁止打开
  setFileClose(false);//禁止关闭
}
...
var codebase=ctxPath+"/resources/OfficeControl.cab#version=5,0,4,0";
var codebase64=ctxPath+"/resources/OfficeControl.cab#version=5,0,4,0";
if (browser=="IE"){
  ...
  document.write('_ProductCaption="江阴市水利局" ');
  document.write('_ProductKey="7C159058552F836EE3FE8EFE474D58E1C191A2CE"');
  ...
  document.write('<SPAN STYLE="color:red">尚未安装NTKO Web FireFox跨浏览器插件。请点击<a href="'+ ctxPath +'/resources/ntko.exe">安装组件</a></SPAN>');
  document.write('</object>   ');
  ...
  document.write('_MenuButtonStyle="7"   ');
  document.write('_WebUserName="NTKO"   ');
  document.write('_Caption="白屈港水利工程运行管理系统">    ');
  document.write('<SPAN STYLE="color:red">尚未安装NTKO Web Chrome跨浏览器插件。请点击<a href="'+ ctxPath +'/resources/ntko.exe">安装组件</a></SPAN>');
  document.write('</object>');
}else if (Sys.opera){
